...
[[nodiscard]] Capabilities make_vulkan_capabilities() {
    return Capabilities{
        ...
        .descriptorPolicy = {
            .mappingModel = NativeDescriptorMappingModel::descriptor_sets,
...
using VulkanBindGroupDescriptorArena = RetainedBindGroupDescriptorArena;
using VulkanBindGroupDescriptorReuseMaterializer =
...
public:
    ...
    [[nodiscard]] Status set_stencil_reference(
        ...
        if (!activeDepthFormat_ ||
            ...
            return Status::failure(StatusCode::invalid_state,
                                   "VulkanCommandBuffer: set_stencil_reference requires stencil-capable depth attachment");
        }
...
public:
    explicit VulkanBuffer(BufferDesc desc)
        : desc_(std::move(desc)),
          storage_(desc_.size),
          mapped_(desc_.mappedAtCreation) {}
    VulkanBuffer(BufferDesc desc, std::byte* externalData)
        : desc_(std::move(desc)),
    ...
    [[nodiscard]] Result<void*> map() override {
        if (!validation::buffer_memory_mappable(desc_)) {
            return Status::failure(StatusCode::unsupported,
                                   "Vulkan buffer memory is not CPU-mappable");
        }
        if (mapped_) {
            return Status::failure(StatusCode::invalid_state,
                                   "Vulkan buffer is already mapped");
        }
        mapped_ = true;
        return data();
    }
...
public:
    ...
    [[nodiscard]] core::Result<std::unique_ptr<IBuffer>> create_buffer(
        ...
        if (desc.mappedAtCreation &&
            ...
            return Status::failure(
                StatusCode::invalid_argument,
                "Vulkan backend: mapped buffer memory must be CPU-mappable");
        }
        if (diagnostics_) {
    ...
    create_bind_group_descriptor_arena(
        ...
        if (!validation::bind_group_descriptor_arena_materialization_valid(
            ...
            return Status::failure(
                StatusCode::invalid_argument,
                "Vulkan backend: descriptor arena materialization is invalid");
        }
    ...
    create_bind_group_descriptor_reuse_materializer(
        ...
        if (!validation::bind_group_descriptor_reuse_materialization_valid(
            ...
            return Status::failure(
                StatusCode::invalid_argument,
                "Vulkan backend: descriptor reuse materialization is invalid");
        }
