...
public:
    ...
    [[nodiscard]] Status draw_indirect(
        ...
        return require_render_pass("draw_indirect");
    }

    [[nodiscard]] Status draw_indexed_indirect(
        IBuffer& /*indirect_buffer*/, std::size_t /*offset*/) override {
        return require_render_pass("draw_indexed_indirect");
    }
...
public:
    Direct3DDevice()
        ...
        caps_.presentation = false;
        caps_.validation = true;
        caps_.maxFramesInFlight = 2;
    }
    ...
    [[nodiscard]] core::Result<std::unique_ptr<ITexture>> create_texture(
        ...
        if (desc.extent.width == 0 || desc.extent.height == 0) {
            return Status::failure(StatusCode::invalid_argument,
                                   "Direct3D backend: texture extent must be non-zero");
        }
    ...
    [[nodiscard]] core::Result<std::unique_ptr<IShader>> create_shader(
        ...
        if (desc.bytecode.empty()) {
            return Status::failure(StatusCode::invalid_argument,
                                   "Direct3D backend: shader bytecode must be present");
        }
    ...
    [[nodiscard]] core::Result<std::unique_ptr<ISurface>> create_surface(
        ...
        if (desc.initialExtent.width == 0 || desc.initialExtent.height == 0) {
            return Status::failure(StatusCode::invalid_argument,
                                   "Direct3D backend: surface extent must be non-zero");
        }
