...
public:
    Direct3DCommandBuffer(BackendDiagnosticsPtr diagnostics, DeviceLimits limits)
    ...
    [[nodiscard]] Status bind_group(
        ...
        if (!validation::bind_group_dynamic_offsets_valid(
            ...
            return Status::failure(StatusCode::invalid_argument,
                                   "Direct3DCommandBuffer: bind group dynamic offsets are invalid");
        }
        ...
        if (!activeLayout) {
            return Status::failure(StatusCode::invalid_state,
                                   "Direct3DCommandBuffer: bind_group requires a bound pipeline");
        }
        if (!validation::pipeline_layout_bind_group_compatible(
                *activeLayout, groupIndex, group.desc().layout->desc())) {
            return Status::failure(
                StatusCode::invalid_argument,
                "Direct3DCommandBuffer: bind group layout is incompatible with pipeline layout");
        }
    ...
    [[nodiscard]] Status set_viewport(
        ...
        if (!validation::viewport_valid(x, y, width, height, minDepth, maxDepth)) {
            return Status::failure(StatusCode::invalid_argument,
                                   "Direct3DCommandBuffer: viewport descriptor is invalid");
        }
        return Status::success();
    }
    ...
    [[nodiscard]] Status set_scissor(
        ...
        if (!validation::scissor_valid(x, y, width, height)) {
            return Status::failure(StatusCode::invalid_argument,
                                   "Direct3DCommandBuffer: scissor rectangle is invalid");
        }
        return Status::success();
    }
    ...
    [[nodiscard]] Status set_stencil_reference(
        ...
        if (!activeDepthFormat_ ||
            ...
            return Status::failure(StatusCode::invalid_state,
                                   "Direct3DCommandBuffer: set_stencil_reference requires stencil-capable depth attachment");
        }
    ...
    [[nodiscard]] Status dispatch_compute(
        ...
        if (!computeLayout_) {
            return Status::failure(StatusCode::invalid_state,
                                   "Direct3DCommandBuffer: dispatch_compute requires a bound compute pipeline");
        }
        if (!validation::pipeline_layout_required_groups_bound(
                *computeLayout_, boundComputeGroups_)) {
            return Status::failure(
                StatusCode::invalid_state,
                "Direct3DCommandBuffer: dispatch_compute requires all pipeline bind groups");
        }
        if (diagnostics_) {
...
private:
    ...
    [[nodiscard]] Status require_graphics_bind_groups(const char* op) const {
        if (!graphicsLayout_) {
            return Status::failure(
                StatusCode::invalid_state,
                std::string{"Direct3DCommandBuffer: "} + op +
...
public:
    explicit Direct3DBuffer(BufferDesc desc)
        : desc_(std::move(desc)),
          storage_(desc_.size),
          mapped_(desc_.mappedAtCreation) {}
    Direct3DBuffer(BufferDesc desc, std::byte* externalData)
        : desc_(std::move(desc)),
    ...
    [[nodiscard]] Result<void*> map() override {
        if (!validation::buffer_memory_mappable(desc_)) {
            return Status::failure(StatusCode::unsupported,
                                   "Direct3D buffer memory is not CPU-mappable");
        }
        if (mapped_) {
            return Status::failure(StatusCode::invalid_state,
                                   "Direct3D buffer is already mapped");
        }
        mapped_ = true;
        return data();
    }
...
public:
    ...
    [[nodiscard]] core::Result<std::unique_ptr<IBuffer>> create_buffer(
        ...
        if (desc.mappedAtCreation &&
            ...
            return Status::failure(
                StatusCode::invalid_argument,
                "Direct3D backend: mapped buffer memory must be CPU-mappable");
        }
        if (diagnostics_) {
    ...
    [[nodiscard]] CommandBufferPtr create_command_buffer() override {
        ...
        return CommandBufferPtr(
            new Direct3DCommandBuffer(diagnostics_, make_direct3d_capabilities().limits),
            [](ICommandBuffer* cmd) {
            delete cmd;
        });
    }
