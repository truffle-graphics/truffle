...
| Vulkan on Linux | Native upload/readback/device-local buffers, host-visible linear images, and capability-checked 1D/2D/3D/cube/array/mipmapped/compressed/multisampled images; compatible views; logical memory budgets; buffer/image and image/image copies; whole-subresource clear; color resolve; nearest/linear color blit. | External sharing, compressed or partial clear, and unsupported runtime format/sample/tiling combinations fail explicitly. Samplers are part of the ordinary-binding capability implemented through #53; RHI 1 exposes no device-address contract. |
...
| OpenGL/OpenGL ES on Linux | Native upload/readback/device-local buffers, samplers, uncompressed color/depth/stencil 2D/array/mip/3D/cube textures, pitched transfers, clears, copies, and nearest/linear blits. Desktop GL also owns core texture views and multisample resolve. | Both profiles reject 1D, compressed, and external textures. GLES additionally rejects texture views, multisample textures/resolve, and cube arrays. |
