...
| Vulkan | Linux | Native resource and transfer smoke slice | `native_smoke` | Pinned private Vulkan-Headers/volk load the host runtime and optionally enable Khronos validation. Native buffers and capability-checked device-local 1D/2D/3D/cube/array/mipmapped/compressed/multisampled images support compatible views, subresource copies, whole-subresource clear, color resolve, and color blit with exact readback evidence. Host-visible textures, external sharing, pipelines, synchronization, WSI, presentation, and full conformance remain unsupported. |
