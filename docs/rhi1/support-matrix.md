...
| Direct3D 12 | Windows | Native Windows-SDK WARP resource, pipeline, and synchronization smoke slice | `native_smoke` | Creates the DXGI WARP adapter and owns resources/transfers, ShaderPackage-backed root signatures, graphics/compute PSOs, draw/dispatch commands, explicit barriers, native timeline fences, and timestamp/occlusion queries. Debug-layer and GPU-based validation check ordered work, query readback, and exact transfer/render/compute output. Compressed/external textures, explicit texture clear/resolve/blit, native multi-queue ownership transfer, DXGI presentation, and full conformance remain unsupported. |
