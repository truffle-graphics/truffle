...
| OpenGL | Linux | Native surfaceless EGL OpenGL resource and direct-graphics smoke slice | `native_smoke` | Creates a real validation-enabled context and pbuffer; owns mapped/device-local buffers, samplers, uncompressed color/depth/stencil 2D/array/mip/3D/cube textures, core texture views, multisample textures, native GLSL shaders, linked programs, and one-target direct drawing. KHR_debug-backed exact tests cover resources plus an exact triangle pixel. Compressed/external textures, 1D textures, advanced bindings/pipeline state, synchronization, window presentation, and full conformance remain unsupported. |
| OpenGL | Windows, macOS | Source target | `source_only` | WGL and deprecated macOS native execution lanes are not present. |
| OpenGL ES | Linux | Native surfaceless EGL ES 3 resource and direct-graphics smoke slice | `native_smoke` | Creates a real validation-enabled ES context and pbuffer; owns mapped/device-local buffers, samplers, core uncompressed color/depth/stencil 2D/array/mip/3D/single-cube textures, native GLSL ES shaders, linked programs, and one-target direct drawing. KHR_debug-backed exact tests cover resources plus an exact triangle pixel. Texture views, multisample textures/resolve, cube arrays, compressed/external and 1D textures, advanced bindings/pipeline state, synchronization, Android surfaces, presentation, and full conformance remain unsupported. |
...
| OpenGL and OpenGL ES textures | [`6b4e916`](https://github.com/truffle-graphics/truffle/commit/6b4e916) via [PR #154](https://github.com/truffle-graphics/truffle/pull/154) | [Build `33964186004`](https://github.com/truffle-graphics/truffle/actions/runs/33964186004) Ubuntu EGL validation lane | Shared uncompressed color/depth/stencil 2D/array/mip/3D/cube ownership, samplers, pitched transfers, clears and blits; desktop GL core views and multisample resolve; separate capability snapshots, exact output, explicit profile negatives, and zero KHR_debug diagnostics. |
| OpenGL and OpenGL ES direct graphics | [`af05f08`](https://github.com/truffle-graphics/truffle/commit/af05f08) via [PR #155](https://github.com/truffle-graphics/truffle/pull/155) | [Build `33965233739`](https://github.com/truffle-graphics/truffle/actions/runs/33965233739) Ubuntu EGL validation lane | Native-source ShaderPackage GLSL/GLSL ES compilation, linked program ownership, one-color framebuffer rendering, direct vertex-ID triangles, exact red center pixels, conservative capability snapshots, and zero KHR_debug diagnostics on both profiles. |
