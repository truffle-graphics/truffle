...
| Vulkan | Linux | Native resource, transfer, and shader-module smoke slice | `native_smoke` | Pinned private Vulkan-Headers/volk load the host runtime and optionally enable Khronos validation. Native buffers, host-visible linear images, and capability-checked 1D/2D/3D/cube/array/mipmapped/compressed/multisampled images support compatible views, subresource copies, whole-subresource clear, color resolve, and color blit with exact readback evidence. Generated ShaderPackage SPIR-V creates owned native modules. External sharing, pipelines, synchronization, WSI, presentation, and full conformance remain unsupported. |
