...
| OpenGL | Linux | Native surfaceless EGL OpenGL resource and transfer smoke slice | `native_smoke` | Creates a real validation-enabled context and pbuffer; owns mapped/device-local buffers, samplers, uncompressed color/depth/stencil 2D/array/mip/3D/cube textures, core texture views, and multisample textures. KHR_debug-backed exact tests cover copies, partial clears, nearest/linear blits, resolve, padded layouts, layers/slices/faces, and memory budgets. Compressed/external textures, 1D textures, pipelines, synchronization, window presentation, and full conformance remain unsupported. |
| OpenGL | Windows, macOS | Source target | `source_only` | WGL and deprecated macOS native execution lanes are not present. |
| OpenGL ES | Linux | Native surfaceless EGL ES 3 resource and transfer smoke slice | `native_smoke` | Creates a real validation-enabled ES context and pbuffer; owns mapped/device-local buffers, samplers, and core uncompressed color/depth/stencil 2D/array/mip/3D/single-cube textures. KHR_debug-backed exact tests cover copies, partial clears, nearest/linear blits, padded layouts, layers/slices/faces, and memory budgets. Texture views, multisample textures/resolve, cube arrays, compressed/external and 1D textures, pipelines, synchronization, Android surfaces, presentation, and full conformance remain unsupported. |
