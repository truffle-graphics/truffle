...
| Direct3D 12 | Windows | Native Windows-SDK WARP resource and transfer smoke slice | `native_smoke` | Creates the DXGI WARP adapter and owns committed buffers, device-local uncompressed single-sample textures, SRV/UAV/RTV/DSV views, per-subresource copy states, and native buffer/texture transfers. Exact padded-row output is checked under the debug layer. Compressed/multisampled/external textures, clear/resolve/blit, pipelines, synchronization, DXGI presentation, and full conformance remain unsupported. |
